...
image_saver=detectify.EuclideanDistTracker()
...
def predict_speed(
    ...
    scale_constant = 10  # manual scaling because we did not performed camera calibration
    ...
    if len(bottom_position_of_detected_vehicle) != 0 and bottom \
        ...
        and roi_position < bottom and (current_frame_number - current_frame_number_list_2[0])>24:
        is_vehicle_detected.insert(0, 1)
        update_csv = True
        #image_saver.capture(crop_img)  # save detected vehicle image
    ...
    if isInROI:
        ...
        total_time_passed = current_frame_number - current_frame_number_list_2[0]
        scale_real_time_passed = total_time_passed *24  # get the elapsed total time for a vehicle to pass through ROI area (0.655 = fps)
        if scale_real_time_passed != 0:
            ...
            speed = abs(int((speed / 6 * 40)))  # use reference constant to get vehicle speed prediction in kilometer unit
            current_frame_number_list_2.insert(0, current_frame_number)
